  ...
  getusers() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (userData['job'] == 'admin') {
        lesuser = lesuser
            .where((lesuser) => lesuser.created_by == userData['id'])
            .toList();
      } else {
        lesuser = ll.map<User>((json) => User.fromJson(json)).toList();
      }
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        tousplannings = tousplannings
            .where((tousplannings) => tousplannings.user_id == iduser)
            .toList();
      });
    }
        ...
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                Container(
                    padding: EdgeInsets.only(right: 50, left: 130),
                    child: Text(
                      ...
                      child: DropdownButtonFormField<dynamic>(
                        ...
                        onChanged: (var val) {
                          ...
                          for (int i = 0; i < lesuser.length; i++) {
                            List iduser = lesuser
                                .where((lesuser) =>
                                    lesuser.firstname == name.split(' ')[0] &&
                                    lesuser.lastname == name.split(' ')[1])
                                .toList();
                            user_id = iduser[0].id;
                          }
                          setState(() {
                            getplannings(user_id);
                          });
                          });
                        },
